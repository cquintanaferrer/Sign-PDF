...
import { signCertificate } from '../services/api'
import { EcdsaProvider, createEcdsaP256Csr } from '../crypto'
...
interface ProcessingScreenProps {
  ...
  userName: string
  userEmail: string
  onComplete: (keys: GeneratedKeys) => void
}

export function ProcessingScreen({ algorithm, token, userName, userEmail, onComplete }: ProcessingScreenProps) {
  ...
  useEffect(() => {
    ...
    let generatedKeys: GeneratedKeys | null = null

    const doBackendWork = async () => {
      try {
        // 1. Generar par de llaves ECDSA P-256 localmente (Web Crypto API)
        const provider = new EcdsaProvider()
        const keyPair = await provider.generateKeyPair()
        const privateKeyPem = await provider.exportPrivateKey(keyPair.privateKey)
        const publicKeyPem = await provider.exportPublicKey(keyPair.publicKey)

        // 2. Construir y firmar el CSR con la llave privada local
        const csrPem = await createEcdsaP256Csr(keyPair.privateKey, keyPair.publicKey, {
          commonName: userName || userEmail,
          email: userEmail,
        })

        // 3. Enviar CSR a nuestro backend
        const cert = await signCertificate(csrPem, token)

        generatedKeys = { privateKeyPem, publicKeyPem, certId: cert.id }
    ...
    const runStep = () => {
      if (stepIndex >= STEPS.length) {
        ...
        const checkFinish = setInterval(() => {
          if (backendFinished && generatedKeys) {
            clearInterval(checkFinish)
            setProgress(100)
            setTimeout(() => onComplete(generatedKeys!), 600)
          }
        }, 500)
        return
      }
