...
export default defineConfig(({ mode }) => {
  // .figma/make/deploy-preview passes `--mode development`
  // for cached-preview builds.
  const emitSourcemaps = mode === 'development'

  return {
    // La aplicación se sirve detrás de Nginx en /client/
    base: '/client/',
...
function figmaSiteConfiguration(
  ...
  const language =
    sanitizeHtmlValue(config.language) || 'en'
...
`,
        ...
        if (
          config.accessibility?.addBypassLinks
        ) {
          tags.push(
            {
              tag: 'style',
...
function figmaErrorOverlayReplay(): Plugin {
  return {
    ...
    configureServer(server) {
      ...
      const origSend =
        server.ws.send.bind(server.ws) as (
          ...args: any[]
        ) => void
...
/**
 * Reload when a module that previously defined a React
 * Refresh boundary stops defining one.
 */
