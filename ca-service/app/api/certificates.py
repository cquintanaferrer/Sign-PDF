...
from app.services.external_chain_verification_service import (
    verify_external_certificate_chain,
)
...
async def verify_certificate_chain(
    ...
    """
    Verifica una cadena X.509 externa utilizando
    el almacén público de CAs de certifi.
    """

    try:
        chain_pem = await chain.read()

        return verify_external_certificate_chain(
            chain_pem=chain_pem,
            hostname=hostname,
        )
        ...
        raise HTTPException(
            status_code=500,
            detail=(
                "Error interno al verificar "
                f"la cadena de certificados: {exc}"
            ),
        ) from exc
